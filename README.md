...
4. **Обогащённая база знаний.** Помимо 7 карточек компаний, в Qdrant загружен корпус
   отраслевых кейсов (боль → решение → эффект), на которые опирается LLM, — рекомендации
   получаются предметными, а не «водой».
...
При `SEED_ON_START=true` (значение по умолчанию в `.env`) база **заполняется
...
### Не забудьте предварительно pip install -r requirements.txt!
```bash
